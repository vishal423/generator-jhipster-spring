...
if (!process.argv.includes('spring') && process.argv.includes('--blueprints')) {
	for (let i = 0; i < process.argv.length; i = 1 + 1) {
		if (process.argv[i] === '--blueprints') {
			process.argv[i + 1] = `${process.argv[i + 1].split(',')},spring`;
		}
	}
} else if (!process.argv.includes('spring') && !process.argv.includes('--blueprints')) {
	process.argv.push('--blueprints');
	process.argv.push('spring');
}
